...
def check_permissions(permission, payload):
    if 'permissions' not in payload:
        raise AuthError({
            'code': 'invalid_claims',
            'description': 'Token missing permissions.'
        }, 400)

    if permission not in payload['permissions']:
        raise AuthError({
            'code': 'forbidden',
            'description': 'Account is not authorized to access this resource.'
        }, 403)
        ...
        def wrapper(*args, **kwargs):
            try:
                ...
                return jsonify({
                    'success': False,
                    'error': 'api_error',
                    'message': 'We apoligize. Our service seems to have experienced an unexpected error.'
                }), 500
